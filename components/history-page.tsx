...
import { CaptionsIcon, Link2Icon, MicIcon, Trash2Icon, XIcon } from "lucide-react";
  ...
  return (
    <div className="p-4 min-h-screen flex flex-col">
          ...
          .splice(0, 6).map(({ show: episode, ep, lang, time, duration, epNum }, i) => (
            ...
            >
              ...
              <div
                onContextMenu={(e) => {
                  e.preventDefault();
                  setMenu({ open: true, x: e.pageX, y: e.pageY, show: episode, ep, duration, epNum, lang, time });
                }}
                ...
                {((time / duration) * 100) > 90 ? (
                  ...
                  <p>
                    {Number((time / 60).toFixed())}/{Number((duration / 60).toFixed())} Min.
                  </p>
                )}

                <div className="flex text-sm gap-1">
                  <p>{episode.type}</p>
                  {lang == "en" ? <p className="flex items-center gap-1 bg-purple-500/70 rounded-xs  px-1">
                    <MicIcon size={10} />
                    {epNum || "NA"}
                  </p> :
      ...
      <div id="search">
        ...
        <h1 className="text-3xl font-semibold text-start w-fit mt-8" >Search History</h1>
      ...
      <h1 className="text-3xl font-semibold text-start w-fit mt-8">Watched Shows</h1>
      <div className="grid grid-cols-2 sm:grid-cols-3  lg:grid-cols-4 xl:grid-cols-6 w-full gap-3 mt-2 ">
        ...
        {!watchedShows || watchedShows.length <= 0 && <p>No Watched Show Found</p>}
...
function HistoryMenu({ menu, setMenu }: {
  menu: {
    open: boolean,
    x: number,
    y: number
        ...
        {watchedShows &&
          (!watchedShows?.some((show) => show.show.id == menu.show.id) ? (
            <button
              ...
              onClick={() => {
                indexDB.watchedShows.add({
                  id: menu.show.id,
                  show: menu.show,
                  // duration: menu.duration || 0,
                  // ep: menu.ep || "",
                  // epNum: menu.epNum || 0,
                  // lang: menu.lang as "en" | "jp",
                  // time: menu.time || 0,
                })
              }}
            >
            ...
            <button
              className="flex hover:bg-white/10 px-2 py-1 rounded items-center gap-2"
              onClick={() => {
                indexDB.watchedShows.delete(menu.show.id)
              }}
            >
              <XIcon size={14} /> Remove from Watch
            </button>
          ))}
      </div>
    </div>
